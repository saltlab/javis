...
/**
 * This class can be used to execute a system command from a Java application. See the documentation
 * for the public methods of this class for more information.
 * 
...
public class SystemCommandExecutor {
	 ...
	 * commands.add("-c"); commands.add("5"); commands.add("www.google.com"); SystemCommandExecutor
	 * commandExecutor = new SystemCommandExecutor(commands); commandExecutor.executeCommand();
	 * 
	 * Note: I've removed the other constructor that was here to support executing the sudo command.
	 * I'll add that back in when I get the sudo command working to the point where it won't hang
	 * when the given password is wrong.
	 * 
	...
	public int executeCommand() throws IOException, InterruptedException {
		...
		try {
			...
			// these need to run as java threads to get the standard output and error from the
			// command.
			...
			// TODO the inputStreamHandler has a nasty side-effect of hanging if the given password
			// is wrong; fix it
